...
makeCacheMatrix <- function(x = matrix()) {
  ...
  if (det(x) == 0) {
    stop("Matrix is singular.  It cannot be inverted")
  }
...
##
##	Inputs:	
##		x	    a "matrix" object generated by function makeCacheMatrix
##		      This object is a list of setters and getters:   
##           list (set="function(y)",  
##                 get="function()", 
##                 setsolve="function(solve)",
##                 getsolve="function()"
...
## Validate input 
  if (!is.list(x)) {
    stop("Invalid input.  Expecting a list object")
  }
  
## Make sure the input is the list object created by makeCacheMatrix
  inList <- names(x)
  if (!(inList[1] == "set"  &&  inList[2] == "get"
        && inList[3] == "setsolve"  && inList[4] == "getsolve")) {
    stop("Invalid input.  Expecting the list object created by function makeCacheMatrix")
  }
  
## Is the inverse for this matrix in cache?
  m <- x$getsolve()
  if (!is.null(m)) {
...
## Inverted matrix not in cache.  Load the matrix
  data <- x$get()
  
## Double-check matrix to be sure it is invertible
  if (det(data) == 0) {
    stop("Matrix is singular.  It cannot be inverted")
  }
